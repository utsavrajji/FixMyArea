...
export default function IssueDetailPage() {
  ...
  const renderSkeleton = () => (
    <div className="space-y-5 px-4 py-6 sm:px-6 mx-auto max-w-5xl">
       <div className="h-10 w-full bg-white rounded-xl shadow-sm mb-6 animate-pulse" />
       ...
       <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
         {[1,2,3,4].map(i => <div key={i} className="h-24 bg-white rounded-2xl shadow-sm animate-pulse" />)}
       </div>
       <ProfileSkeleton />
    </div>
  );
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center pt-20">
              ...
              <button onClick={() => navigate("/admin-dashboard")} className="mt-4 text-[#064E3B] font-bold">Back to Dashboard</button>
            </div>
          ) : (
            <motion.div
              key="content"
            ...
            >
              ...
              <div className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-gray-200 bg-white px-4 shadow-sm sm:px-6">
                ...
                <div className="flex items-center gap-2">
                  <span className={`hidden rounded-full border px-3 py-1 text-xs font-semibold sm:inline-flex ${STATUS_CONFIG[issue.status]?.active || "bg-gray-100 text-gray-600"}`}>
                    {issue.status}
                  </span>
                  <button onClick={() => setShowEmailForm(true)}
                    ...
                    <Mail className="w-4 h-4" /> Send to Govt
                  </button>
                </div>
              </div>
              ...
              <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 space-y-5">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
                  <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${STATUS_CONFIG[issue.status]?.color || "bg-gray-100"}`}>
                      {issue.status}
                    </span>
                  </div>
                  <p className="text-sm leading-relaxed text-gray-700">{issue.description || "No description provided."}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                  {[
                    { icon: <Folder className="w-5 h-5 text-emerald-600" />, label: "Category", value: issue.category },
                    { icon: <Search className="w-5 h-5 text-blue-600" />, label: "Sub-Issue", value: issue.subIssue || "—" },
                    { icon: <MapPin className="w-5 h-5 text-red-600" />, label: "Location", value: typeof issue.location === "object" ? Object.values(issue.location).filter(Boolean).join(", ") : issue.location },
                    { icon: <ThumbsUp className="w-5 h-5 text-amber-600" />, label: "Upvotes", value: `${issue.upvotes || 0}` },
                  ].map(({ icon, label, value }) => (
                    <div key={label} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                      <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-gray-400">{icon} {label}</div>
                  ...
                  {(issue.status === "Fake" || issue.status === "Not Important") && (
                    <div className="mt-5 border-t border-red-100 pt-5">
                      <button onClick={handleDelete} disabled={updating}
                        ...
                        <Trash2 className="w-4 h-4" /> Delete This Issue Permanently
                      </button>
