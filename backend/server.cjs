...
app.use(cors({ 
  origin: ['https://fix-my-area-seven.vercel.app', 'http://localhost:3000'], 
...
// Initialize Resend (HTTP-based email API - works on Render)
...
app.post("/api/send-to-government", async (req, res) => {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'FixMyArea <onboarding@resend.dev>', // Resend verified sender
      to: [governmentEmail],
...
app.get("/", (req, res) => {
  res.json({ 
    ...
    emailService: "Resend API (HTTP-based, works on Render)",
    version: "2.0.0"
  });
});
